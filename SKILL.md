...
**If an API key is still missing after install:** use `AskUserQuestion` to ask the user whether they have a Groq API key (preferred — cheaper, faster), an OpenAI key, or an OpenRouter key. Then write it into `~/.config/watch/.env` — set the matching `GROQ_API_KEY=...`, `OPENAI_API_KEY=...`, or `OPENROUTER_API_KEY=...` line. If they don't want to set up Whisper, proceed with `--no-whisper` and tell them videos without native captions will come back frames-only.
...
- `--whisper groq|openai|openrouter` — force a specific Whisper backend (default: prefer Groq, then OpenAI, then OpenRouter)
...
2. **Whisper API fallback.** If no captions came back (or the source is a local file), the script extracts audio (`ffmpeg -vn -ac 1 -ar 16000 -b:a 64k`, ~0.5 MB/min) and uploads it to whichever Whisper API has a key configured:
   - **Groq** — `whisper-large-v3`. Preferred default: cheaper, faster, returns per-segment timestamps. Get a key at console.groq.com/keys.
   - **OpenAI** — `whisper-1`. Returns per-segment timestamps. Get a key at platform.openai.com/api-keys.
   - **OpenRouter** — `openai/whisper-large-v3-turbo` (Groq under the hood; same $0.04/hr). Useful when you already have an OpenRouter key for chat/vision and want one bill. **Caveat:** OpenRouter's `/audio/transcriptions` endpoint returns plain text only (no per-segment timestamps), so the transcript is delivered as a single segment without timeline alignment. For timestamp-aware questions, prefer Groq or OpenAI. Get a key at openrouter.ai/keys.

All three keys live in `~/.config/watch/.env`. The script prefers Groq → OpenAI → OpenRouter when multiple are set; override with `--whisper openai` or `--whisper openrouter` to force a specific backend. Use `--no-whisper` to skip the fallback entirely.
...
- Sends the extracted audio clip to OpenRouter's audio transcription endpoint (`openrouter.ai/api/v1/audio/transcriptions`) when `OPENROUTER_API_KEY` is set and the higher-priority backends are not, or when `--whisper openrouter` is forced. OpenRouter routes the request to its `openai/whisper-large-v3-turbo` provider (currently Groq).
...
- Does not share API keys between providers (Groq key only goes to `api.groq.com`, OpenAI key only goes to `api.openai.com`, OpenRouter key only goes to `openrouter.ai`)
